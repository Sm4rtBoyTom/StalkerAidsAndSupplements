...
namespace StalkerAidsAndSupplementsMod.Afflictions
{
    internal class AfflictionPatches
    {
        ...
        private static class CraftingSpeedPatch
        {
            ...
            {
                float multiplier = AfflictionLogic.GetCraftingSpeedMultiplierUI();

                __result = Mathf.CeilToInt(__result * multiplier);
            }
        }
              [HarmonyPatch(typeof(CraftingOperation), nameof(CraftingOperation.ApplyCraftingProgress))]
              private static class CraftingSpeedPatchTwo
              {
                  private static void Prefix(ref float hoursSpentCrafting)
                  {
                      hoursSpentCrafting *= AfflictionLogic.GetCraftingSpeedMultiplier();
                  }
              }
              [HarmonyPatch(typeof(CraftingOperation), nameof(CraftingOperation.ConsumeMaterialsUsedForCrafting))]
              private static class CraftingSpeedPatchThree
              {
                  private static void Prefix(ref float hoursSpentCrafting)
                  {
                    hoursSpentCrafting *= AfflictionLogic.GetCraftingSpeedMultiplier();
        ...
        [HarmonyPatch(typeof(CraftingOperation), nameof(CraftingOperation.Update))]
         class CraftingOperation_Update
         {
             static void Postfix(CraftingOperation __instance)
             {
                 if (!CigaretteBuff.ShouldInterrupt)
                     return;

                 CigaretteBuff.ShouldInterrupt = false;

                 __instance.Interrupt(CraftingInterruptReason.Cancelled);
             }
         }

        [HarmonyPatch(typeof(Panel_BreakDown), nameof(Panel_BreakDown.UpdateDurationLabel))]
        private static class BreakDownPatch
        {
            private static void Postfix(Panel_BreakDown __instance)
            {
                if (__instance == null) return;

                __instance.m_DurationHours *= AfflictionLogic.GetCraftingSpeedMultiplierUI(); 
        ...
        private static class CarcassHarvestPatch
        {
            private static void Postfix(ref float __result)
            {

                __result *= AfflictionLogic.GetCraftingSpeedMultiplierUI();
            }
        }

        [HarmonyPatch(typeof(Panel_BodyHarvest), nameof(Panel_BodyHarvest.GetQuarterDurationMinutes))]
        private static class CarcassQuarterPatch
        {
        ...
        internal static class InventoryRepairDurationPatch
        {
            ...
            {
                if (!AfflictionLogic.IsInventoryRepairContext(__instance)) return;

                AfflictionLogic.ApplyActionTimetoMinutes(ref __result, "RepairDuration");

                if (__instance == null) return;

                __instance.m_Repair_TimeLabel.text = Utils.GetExpandedDurationString(Mathf.RoundToInt(__result));
            }
        }
